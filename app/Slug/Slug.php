...
class Slug
{
    ...
    /**
     * Separator used to generate slugs
     *
     * @var string
     */
    const SEPARATOR = '-';
    ...
    public function __construct($value)
    {
     ...
     {
         // Create the slug and use the defined separator
         $slug = str_slug($this->initialValue, static::SEPARATOR);

         // Find any similar slugs and add to the banned slugs array
         $notAllowed = $this->getSimilarSlugs($slug)->merge($this->banned);

         // Check if the slug is not allowed
         if ($notAllowed->isEmpty() || !$notAllowed->contains($slug)) {
             return $slug;
         }

         // Generate a suffix to make the slug unique
